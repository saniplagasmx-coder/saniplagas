  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${
        scrolled
          ...
          : 'bg-white/10 backdrop-blur-[2px] py-4'
      }`}
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        {/* Desktop Links */}
        <div className="hidden md:flex items-center gap-2 bg-black/5 p-1 rounded-2xl backdrop-blur-md" role="menubar">
          {navLinks.map((link) => (
            <button
              key={link.href}
      ...
      <div
        id="mobile-menu"
        role="menu"
        className={`md:hidden transition-all duration-500 ease-in-out overflow-hidden ${menuOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}
      >
